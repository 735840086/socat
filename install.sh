...
PATH_SOCAT="/root/${APP_ID}"
PATH_EXEC="${APP_ID}"
DOWNLOAD_EXEC_PREFIX="${PATH_EXEC}"
ORIGIN_EXEC="${DOWNLOAD_EXEC_PREFIX}-${VERSION}"
CLI_EXEC_NAME="socatSystemCLI-0.1.0"
CLI_COMMAND_NAME="socatminer"
CLI_LINK_PATH="/usr/local/bin/${CLI_COMMAND_NAME}"
CLI_LAUNCHER_MARKER="Generated by socatSystem install.sh"
...
load_language() {
    ...
    2)
        ...
        msg_cli_main_missing="socatSystem is not installed. Please choose 1 to install socatSystem first, then install the CLI tool."
        msg_cli_already_installed="socat CLI is already installed."
        msg_cli_command_exists="socat command already exists and is not managed by this script"
        msg_cli_install_done_title="CLI tool installed successfully"
        msg_cli_install_done_run="Run socat to open the CLI tool. Use socat start, socat stop, or socat restart to control socatSystem."
        ...
        msg_cli_install_warning_title="Before installing the CLI tool, socatSystem WEB access must be disabled."
        ...
        msg_cli_not_installed="socat CLI is not installed."
        msg_cli_uninstall_done="CLI tool uninstalled"
        msg_web_ui_change_notice_title="Changing WEB access will restart socatSystem."
        msg_web_ui_change_notice_detail="If WEB access is disabled, the web page will be unreachable; use the socat CLI to access statistics. If the CLI is not installed, install it from this script's menu."
        msg_web_ui_change_continue="1. Continue and restart socatSystem"
        ...
        msg_web_ui_disabled_start_tip="WEB access is disabled. Run socat to open the CLI tool, or enable WEB access from this install script."
    ...
    1)
        ...
        prompt_error_command="错误指令, 重新输入。"
        prompt_set_port="输入端口号："
        prompt_https_question="是否开启HTTPS后台访问"
        prompt_https_off="1. 不开启"
        prompt_https_on="2. 开启"
        prompt_https_choose="请选择 [1-2]："
        prompt_https_invalid="输入错误, 默认开启。"
        prompt_enter_version="无效操作"
        prompt_confirm_uninstall="请输入 YES 确认卸载："
        prompt_confirm_cli_install="无效操作"
        prompt_confirm_cli_uninstall="无效操作"
        prompt_confirm_web_ui_change="无效操作"

        menu_group_install="⭐️部署"
        menu_group_runtime="⭐️控制"
        menu_group_settings="⭐️配置"
        menu_group_logs="⭐️日志"
        menu_group_maintenance="⭐️维护"
        menu_group_cli="⭐️WeChat:735840086"
        menu_tip="⭐️提示：解除系统连接数限制后需手动重启服务器生效"

        msg_install_start="开始部署: "
        msg_disable_firewall="关闭防火墙"
        msg_unknown_os_firewall="未知系统, 关闭防火墙失败"
        msg_starting="启动程序"
        msg_already_running="程序已启动，请误重复启动。"
        ...
        msg_tip_label="   ⭐提示"
        msg_public_firewall_tip="访问WEB后台需打开端口关闭防火墙"
        msg_default_security_tip="部署成功后及时修改控制台账号密码WEB端口"
        msg_http_tip="⭐️WEB协议HTTP"
        msg_https_tip="⭐️WEB协议HTTPS"
        msg_delete_config="删除配置文件"
        msg_password_reset_done="重置密码完成"
        ...
        msg_limit_changed_restart="连接数限制已修改65535,重启服务器后生效"
        msg_current_limit="当前连接数限制："
        msg_change_done_restart="修改完成, 重启服务器后生效"
        msg_running_detected="发现正在运行"
        msg_must_stop_before_install=", 需停止才可继续安装。"
        msg_enter_stop_running="输入1停止"
        msg_or_cancel_install="并继续安装, 输入2取消。"
        msg_cancel_install="取消安装"
        msg_invalid_cancel_install="输入错误, 取消安装。"
        msg_create_dir="开始创建目录"
        msg_dir_exists="目录存在, 无需创建, 继续部署。"
        msg_downloading="开始下载..."
        msg_download_program="下载程序"
        msg_download_source="拉取地址"
        msg_download_target="保存位置"
        msg_download_complete="下载完成"
        msg_uninstall_done="卸载完成"
        msg_cli_install_start="错误"
        msg_cli_download_program="错误"
        msg_cli_main_missing="错误"
        msg_cli_already_installed="错误"
        msg_cli_command_exists="错误"
        msg_cli_install_done_title="错误"
        msg_cli_install_done_run="错误"
        msg_cli_install_done_path="错误"
        msg_cli_install_done_link="错误"
        msg_cli_install_done_doc="错误"
        msg_cli_install_warning_title="错误"
        msg_cli_install_warning_detail="关闭 WEB 访问后，网页将不可达"
        msg_cli_install_continue="错误"
        msg_cli_install_cancel="2. 取消"
        msg_cli_install_cancelled="错误"
        msg_cli_launching="错误"
        msg_cli_not_installed="错误"
        msg_cli_uninstall_done="错误"
        msg_web_ui_change_notice_title="开启或关闭 WEB 访问会重启 socatSystem"
        msg_web_ui_change_notice_detail="关闭 WEB 访问后，网页端将不可访问"
        msg_web_ui_change_continue="1. 继续并重启 socatSystem"
        ...
        msg_web_ui_disabled_start_tip="当前 WEB 访问已关闭"
        msg_tail_hint="日志查看"
...
create_cli_launcher() {
    ...
    if [ "$lang_choice" = "2" ]; then
        text_usage="用法: socat [start|stop|restart|help]"
        text_unknown="未知命令"
        text_need_root="请使用 root 用户运行该控制命令。"
        text_app_missing="未找到 socatSystem 主程序，请先完成安装。"
        text_cli_missing="未找到 socatSystem CLI，请重新安装命令行工具。"
        text_cli_requires_web_disabled="命令行工具仅在 WEB 访问关闭时可用。请先运行安装脚本并选择关闭 WEB 访问。"
        text_already_running="socatSystem 已在运行。"
        text_started="SocatSystem 已启动。"
        text_start_failed="SocatSystem 启动失败。"
        text_not_running="SocatSystem 未运行。"
        text_stopped="SocatSystem 已停止。"
        text_stop_timeout="SocatSystem 停止超时。"
    else
        text_usage="Usage: socat [start|stop|restart|help]"
        text_unknown="Unknown command"
        text_need_root="Please run this control command as root."
        text_app_missing="SocatSystem is not installed. Please install it first."
        text_cli_missing="SocatSystem CLI was not found. Please reinstall the CLI tool."
        ...
        text_already_running="SocatSystem is already running."
        text_started="SocatSystem started."
        text_start_failed="SocatSystem start failed."
        text_not_running="SocatSystem is not running."
        text_stopped="SocatSystem stopped."
        text_stop_timeout="SocatSystem stop timeout."
    fi
...
show_cli_install_success() {
    ...
    echo -e "${msg_cli_install_done_doc}: ${BOLD}${GREEN}https://minerproxy.org/zh/document/socatsystem-cli${RESET}"
...
install_cli() {
    ...
    if ! is_socatsystem_installed; then
...
show_runtime_status() {
    ...
    if [ "$lang_choice" = "2" ]; then
        ...
        label_title="SocatSystem Runtime Status"
        label_basic="Basic"
