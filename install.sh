...
load_language() {
    ...
    1)
        ...
        msg_web_ui_change_notice_detail="关闭 WEB 访问后，网页端将不可访问；可使用 socat 命令行工具访问统计数据。如需安装命令行工具，请在本安装脚本菜单中选择安装命令行工具。"
        ...
        msg_web_ui_disabled_start_tip="当前 WEB 访问已关闭。运行 socat 可打开命令行工具，如需网页访问请在安装脚本中开启 WEB 访问。"
