...
load_language() {
    ...
    1)
        ...
        msg_http_tip="当前后台为HTTP协议"
        msg_https_tip="当前后台为HTTPS协议"
        msg_delete_config="删除配置文件"
        msg_password_reset_done="重置密码完成, 默认账号密码 qzpm19kkx xloqslz913"
        ...
        msg_cli_main_missing="socatSystem 尚未安装，请先选择 1 安装 socatSystem，再安装命令行工具。"
        msg_cli_already_installed="socat 命令行工具已安装。"
        msg_cli_command_exists="socat 命令已存在，且不是由此脚本管理"
        msg_cli_install_done_title="命令行工具安装成功"
        msg_cli_install_done_run="运行 socat 命令即可打开命令行工具；运行 socat start、socat stop、socat restart 可启动、停止、重启 socatSystem。"
        msg_cli_install_done_path="命令行工具路径"
        msg_cli_install_done_link="命令启动脚本"
        msg_cli_install_done_doc="命令行工具"
        msg_cli_install_warning_title="安装命令行工具前，需要关闭 socatSystem 的 WEB 访问。"
        ...
        msg_cli_not_installed="socat 命令行工具未安装。"
